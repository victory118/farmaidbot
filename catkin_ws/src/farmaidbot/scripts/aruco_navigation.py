...
import tf
import threading
from aruco_localization import ArucoLocalization
...
class ArucoNavigation(object):
    ...
    def __init__(self, tag_poses, poselist_base2cam, pose_init, wheelbase, wheel_radius):
        ...
        self._aruco_localization = ArucoLocalization(tag_poses, poselist_base2cam)
    ...
    def process_measurements(self, goal):
        ...
        tag_meas = self._aruco_localization.get_measurement() # calculated robot pose(s) based on Apriltag detections (x, y, theta)
...
if __name__ == "__main__":
    rospy.init_node('aruco_navigation', anonymous=True)
    ...
    rate = rospy.Rate(30)
    ctrl_c = False
    aruco_nav = ArucoNavigation(tag_poses, poselist_base2cam, pos_init, wheelbase, wheel_radius)
    ...
    while not ctrl_c:
        ...
        # print "goal = ", goal
        at_goal = aruco_nav.process_measurements(goal)    
